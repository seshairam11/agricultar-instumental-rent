...
export const OPTextBox = ({ ctl_Attribute }) => {
    ...
    return (
        <>
            <label htmlFor={ctl_Attribute.csstheme.id} className="contact__form--label ">{ctl_Attribute.csstheme.labletext}{ctl_Attribute.validate.mandatory ? (<span className="contact__form--label__star" > &nbsp;*</span>) : null}</label>
            <input
                ref={(el) => {
                    ctl_Attribute.tooltip.isfocus = el
                    ctl_Attribute.tooltip.errorshow = el
                }}
                className={`contact__form--input ${ctl_Attribute.csstheme?.classname} ${ctl_Attribute.error.errorshow ? "err-border" : ""}`}
                type="text"
